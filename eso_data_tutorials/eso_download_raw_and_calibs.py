...
print()
print("Looking for SCIENCE frames belonging to the 086.D-0165(A) observing run." )
print("Querying the ESO TAP service at %s" %(ESO_TAP_OBS))

query="""SELECT top 4 dp_id
FROM dbo.raw
WHERE prog_id='086.D-0165(A)'
AND dp_cat='SCIENCE'
...
print("Downloading the non-proprietary files (%d files) and their calibration cascades (trying with %s mode)." % (len(rawframes.to_table()), mode_requested))
...
for raw in rawframes:
    ...
    # ---------------------------------------
    sci_url = "http://archive.eso.org/datalink/links?ID=ivo://eso.org/ID?%s&eso_download=file" % raw["dp_id"].decode()
